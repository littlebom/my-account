...
export default function EditVendorPage() {
    ...
    const fetchVendor = async (id: string) => {
        try {
            setFetching(true);
            const data = await api.get<any>(`/contacts/${id}?type=vendor`);
            setFormData({
                name: data.name || "",
                taxId: data.taxId || "",
                phone: data.phone || "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.name) {
            alert("กรุณาระบุชื่อเจ้าหนี้");
            return;
        }

        try {
            setLoading(true);
            await api.put(`/contacts/${params.id}`, { ...formData, type: "vendor" });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
                <Link href="/purchase/vendors" className="p-2 rounded-full hover:bg-white text-gray-500 transition-colors">
                    <ChevronLeft size={24} />
                </Link>
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">แก้ไขข้อมูลเจ้าหนี้</h1>
            ...
            <form onSubmit={handleSubmit} className="bg-white shadow-sm border border-gray-200 rounded-xl p-6 space-y-6">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                        <label className="block text-sm font-medium text-gray-700 mb-1">ชื่อบริษัท / ชื่อเจ้าหนี้ <span className="text-red-500">*</span></label>
                        <Input
                            type="text"
                            name="name"
                            required
